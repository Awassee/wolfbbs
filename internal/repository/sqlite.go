...
func (r *SQLiteAdminRepository) DeleteFileEntry(id int64) error {
	...
	res, err := tx.ExecContext(context.Background(), `DELETE FROM file_entries WHERE id = ?`, id)
	if err != nil {
		return err
	}
	...
	if _, err := tx.ExecContext(context.Background(), `DELETE FROM file_ratings WHERE file_id = ?`, id); err != nil {
		return err
	}
	if _, err := tx.ExecContext(context.Background(), `DELETE FROM download_queue WHERE file_id = ?`, id); err != nil {
		return err
	}
	if _, err := tx.ExecContext(context.Background(), `DELETE FROM download_tickets WHERE file_id = ?`, id); err != nil {
		return err
	}
