# WolfBBS Documentation Hub

This directory is organized for three common audiences: operators, contributors, and evaluators.

## Most-Used Pages

- [START_HERE.md](START_HERE.md): fastest path from install to usable system
- [FIRST_30_MINUTES.md](FIRST_30_MINUTES.md): first operator runbook after install
- [INSTALL.md](INSTALL.md): installer modes, upgrade, uninstall, troubleshooting flags
- [SHOWCASE.md](SHOWCASE.md): product tour for GitHub visitors and release notes
- [ACCEPTANCE_SPEC.md](ACCEPTANCE_SPEC.md): acceptance contract and MUST checks
- [manual-acceptance-latest.md](manual-acceptance-latest.md): latest acceptance run report
- [releases/v2.1.9.md](releases/v2.1.9.md): current distro release notes

## Start Here

- [START_HERE.md](START_HERE.md): shortest path from install to a working board
- [QUICKSTART.md](QUICKSTART.md): first-login setup flow
- [INSTALL.md](INSTALL.md): install/uninstall/upgrade command reference
- [TROUBLESHOOTING.md](TROUBLESHOOTING.md): symptom-first recovery guide

## Product And Positioning

- [PRODUCT_GUIDE.md](PRODUCT_GUIDE.md): what WolfBBS is and how to run it as a product
- [SHOWCASE.md](SHOWCASE.md): feature tour with screenshot links
- [DATASHEET.md](DATASHEET.md): concise deployment and capability summary
- [feature-reference.md](feature-reference.md): route/surface inventory
- [OPEN_SOURCE.md](OPEN_SOURCE.md): MIT license, contribution terms, and third-party license notes
- [../SECURITY.md](../SECURITY.md): public vulnerability disclosure and supported release policy

## Operator Playbooks

- [LAUNCH_CHECKLIST.md](LAUNCH_CHECKLIST.md): launch readiness pass
- [OPERATOR_PLAYBOOK.md](OPERATOR_PLAYBOOK.md): day-to-day operations rhythm
- [FIRST_30_MINUTES.md](FIRST_30_MINUTES.md): first operator half-hour checklist
- [RUNNING_A_COMMUNITY.md](RUNNING_A_COMMUNITY.md): community cadence and retention playbook
- [OPERATIONS.md](OPERATIONS.md): production maintenance and incident handling
- [POST_RELEASE_TRIAGE.md](POST_RELEASE_TRIAGE.md): first-72-hours release response loop
- [NEXT_FEATURE_WAVE.md](NEXT_FEATURE_WAVE.md): next major roadmap after hardening
- [admin.md](admin.md): admin panels and workflows

## Feature Guides

- [chat.md](chat.md): web chat and moderation workflows
- [irc-compat.md](irc-compat.md): IRC compatibility and bridge notes
- [doors.md](doors.md): door runtime and scoring
- [message-network.md](message-network.md): message network behavior
- [email-gateway.md](email-gateway.md): inbound/outbound gateway controls
- [web-gateway.md](web-gateway.md): fetch gateway behavior and safety
- [content-servers.md](content-servers.md): gopher/NNTP server setup

## Product Quality And Acceptance

- [ACCEPTANCE_SPEC.md](ACCEPTANCE_SPEC.md): acceptance contract source of truth
- [manual-acceptance.md](manual-acceptance.md): manual acceptance workflow
- [manual-acceptance-latest.md](manual-acceptance-latest.md): latest acceptance report
- [UI_COVERAGE_MATRIX.md](UI_COVERAGE_MATRIX.md): route-level interface coverage
- [END_USER_COVERAGE_AUDIT.md](END_USER_COVERAGE_AUDIT.md): caller-loop parity audit for web vs terminal
- [OPERATIONS.md](OPERATIONS.md): includes the one-command `scripts/feature-complete.sh` readiness path

## Architecture And Planning

- [architecture.md](architecture.md): system architecture
- [architecture-review.md](architecture-review.md): architecture review notes
- [threat-model.md](threat-model.md): security threat model
- [prd/README.md](prd/README.md): PRD set and feature matrix
- [enigma-port-plan.md](enigma-port-plan.md): ENiGMA modernization mapping

## Release Notes

- [releases/](releases): release-by-release change history
- [releases/README.md](releases/README.md): release index + current recommended tag
